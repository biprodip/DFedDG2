...
from learners.learner_cont import *

from learners.learners_ensemble import *
from learners.learners_ensemble_cont import *
...
def get_learner(
    ...
    elif name in ["cifar10","digit","office","domainnet"]:
        criterion = nn.CrossEntropyLoss(reduction="none").to(device)
        metric = accuracy
...
def get_learner_cont(
    ...
    if name == "cifar10":
        ...
        model = get_mobilenet(n_classes).to(device)
        feat_dim = 1280
        # print('Using mobilenet from utils.py')
        head = copy.deepcopy(model.classifier)
        model.classifier = nn.Identity() #freeze the head from original model
        model = BaseHeadSplit(model, head)

        # model = get_resnet18(n_classes=10).to(device)        
        # head = copy.deepcopy(model.fc)
        # model.fc = nn.Identity() #freeze the head from original model
        # model = BaseHeadSplit(model, head)
        ...
        # Define the FemnistCNN model
        femnist_model = FemnistCNN(n_classes).to(device)
        feat_dim = 2048

        # Split the model into base (conv layers) and head (fully connected layers)
        head = copy.deepcopy(femnist_model.output)  # Copy the head (classification layer)
        femnist_model.output = nn.Identity()  # Remove the head from the original model

        # Use BaseHeadSplit to combine base and head
        model = BaseHeadSplit(femnist_model, head).to(device)
...
def get_learners_ensemble(
    ...
    if client_type == "mixture_cont":
        return LearnersEnsembleCont(learners=learners, learners_weights=learners_weights)
    else:
...
def get_client(
    ...
    elif client_type == "mixture_cont":
        return MixtureClientCont(
            learners_ensemble=learners_ensemble,
            train_iterator=train_iterator,
            val_iterator=val_iterator,
            test_iterator=test_iterator,
...
def get_aggregator(
    ...
    if aggregator_type == "no_communication":
        ...
        print('Centralized aggregator used.')
    ...
    elif aggregator_type == "centralized_cont":
        print('Centralized cont aggregator used.')
        return CentralizedAggregatorCont(
            clients=clients,
            global_learners_ensemble=global_learners_ensemble,
            log_freq=log_freq,
            global_train_logger=global_train_logger,
