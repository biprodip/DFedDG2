...
class CompLoss(nn.Module):
    ...
    def forward(self, features, prototypes, labels):
        '''receiving normalized protos from disloss'''
        #prototypes = F.normalize(prototypes, dim=1)



        proxy_labels = torch.arange(0, self.args.num_classes).to(self.args.device)
        labels = labels.contiguous().view(-1, 1)
        mask = torch.eq(labels, proxy_labels.T).float().to(self.args.device) #bz, cls
...
class DisLoss(nn.Module):
    ...
    def forward(self, features, labels):    
        ...
        for j in range(len(features)):
            ...
            #prototypes[labels[j].item()] = prototypes[labels[j].item()] *self.args.proto_m + features[j]*(1-self.args.proto_m)
        self.prototypes = prototypes.detach()
        labels = torch.arange(0, num_cls).to(self.args.device)
        labels = labels.contiguous().view(-1, 1)
