...
class CompLoss(nn.Module):
    ...
    def forward(self, features, prototypes, labels):
        ...
        mask = torch.eq(labels, proxy_labels.T).float().to(self.args.device) #bz x cls    # [[0010000000],[0000010000],[0001000000]]
...
class DisLPLoss(nn.Module):
    ...
    def compute(self):
        ...
        labels = torch.arange(0, num_cls).to(args.device)
        labels = labels.contiguous().view(-1, 1)
    ...
    def init_class_prototypes(self):
        ...
        with torch.no_grad():
            prototypes = torch.zeros(self.args.num_classes,self.args.feat_dim).to(args.device)
...
class DisLossOod(nn.Module):
    '''
    DisLoss on only ID labels and protos.
    Mask out OOD labels using additional mask.
    '''
    def __init__(self, args, model, loader, temperature= 0.1, base_temperature=0.1):
        super(DisLossOod, self).__init__()
        self.args = args
        self.temperature = temperature
        self.base_temperature = base_temperature
        self.register_buffer("prototypes", torch.zeros(self.args.num_classes,self.args.feat_dim))
    ...
    def forward(self, features, labels, id_labels):    
        ...
        # ood_mask = torch.ones(self.args.num_classes,self.args.num_classes).cuda()
        # #set the elements at all indices ood_labels to zero
        # indices_to_zero = torch.tensor(ood_labels).cuda()
        # ood_mask[:,indices_to_zero] = 0 #ignore relations of C^id with C^ood (We encourage distacne within id protos)
        # ood_mask[indices_to_zero,:] = 0 #ignore relations of C^ood with others


        # mask = mask * ood_mask
        
        tmp_logits = mask*logits #for every prptos consider relation with other protos, (masking out self) 
        # non_zero_ind =  [l for l in range(self.args.num_classes) if l not in ood_labels]
        #logits = tmp_logits[non_zero_ind] #id logits extracted
        logits = tmp_logits[id_labels][:,id_labels] #extract only id proto relations
        id_mask = mask[id_labels][:,id_labels]   
        # print(logits.shape)

        # mean_prob_neg = torch.log((mask * torch.exp(logits)).sum(1) / mask.sum(1))
        mean_prob_neg = torch.log(torch.exp(logits).sum(1) / id_mask.sum(1))
        mean_prob_neg = mean_prob_neg[~torch.isnan(mean_prob_neg)]
        loss = self.temperature / self.base_temperature * mean_prob_neg.mean()
        return loss
