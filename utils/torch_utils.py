    ...
    for key in target_state_dict:
        ...
        if target_state_dict[key].data.dtype == torch.float32:
            ...
            for learner_id, learner in enumerate(learners):
                ...
                if average_params:
                    target_state_dict[key].data += weights[learner_id] * state_dict[key].data.clone()  #######this is key
...
def aggregate_prototypes(clients_prototypes, clients_weights):
    """
    Aggregates prototypes from all clients by weighted average.

    Parameters:
    - clients_prototypes: List of prototypes from each client. Each entry is a tensor of shape [num_classes, feature_dim].
    - clients_weights: List of client weights based on the number of samples or other criteria.
    
    Returns:
    - aggregated_prototypes: Tensor of shape [num_classes, feature_dim].
    """
    num_clients = len(clients_prototypes)
    num_classes = clients_prototypes[0].shape[0]  # Assuming all clients have the same number of classes
    feature_dim = clients_prototypes[0].shape[1]
    
    # Initialize a tensor for aggregated prototypes
    aggregated_prototypes = torch.zeros(num_classes, feature_dim).to(clients_prototypes[0].device)

    # Weighted sum of prototypes from clients
    for client_idx in range(num_clients):
        aggregated_prototypes += clients_weights[client_idx] * clients_prototypes[client_idx]

    # Normalize the aggregated prototypes
    aggregated_prototypes = F.normalize(aggregated_prototypes, dim=1)
