...
class Aggregator(ABC):
    ...
    def update_test_clients(self):
        ...
        for client in self.test_clients:
            ...
            print('Sample weights updated in test clients')
            client.update_learners_weights()
            print('Learners weights updated in test clients')

    def write_logs(self, num_trials=1, curr_trial=0):
            ...
            for client_id, client in enumerate(clients):
                ...
                if self.verbose > 1:
                    ...
                    if num_trials == 1:
                        ...
                        print(f"Trial {curr_trial} Train Loss: {train_loss:.3f} | Train Acc: {train_acc * 100:.3f}%|", end="")
                        print(f"Trial {curr_trial} Test Loss: {test_loss:.3f} | Test Acc: {test_acc * 100:.3f}% |")
...
class CentralizedAggregatorCont(Aggregator):
    r""" Standard Centralized Aggregator.
     All clients get fully synchronized with the average client.
    ...
    def mix(self):
        ...
        for client in self.sampled_clients:
            client.step()  # q,pi, theta, mu and assign mus in the learner_ensemble.prototypes of clients

        #########################
        clients_prototypes = [client.learners_ensemble.prototypes[:,0,:] for client in self.sampled_clients]  # Assumes `get_prototypes()` is defined in the client ##self.clienys
        clients_weights = [client.n_train_samples / sum(client.n_train_samples for client in self.sampled_clients) for client in self.sampled_clients]##self.clienys

        # Aggregate prototypes using the server-side aggregation function
        global_prototypes = aggregate_prototypes(clients_prototypes, clients_weights)
        print(f'Aggregated global prototypes: {global_prototypes}')
        
        # Broadcast global prototypes to all clients
        for client in self.sampled_clients:
            client.update_prototypes_from_server(global_prototypes)
        ################################
    ...
    def broadcast_prototypes_to_clients(self, aggregated_prototypes):
        """
        Broadcast the aggregated prototypes to all clients.
        
        :param aggregated_prototypes: Tensor of shape [num_classes, feature_dim].
        """
        for client in self.sample_clients:
            client.update_prototypes_from_server(aggregated_prototypes)
