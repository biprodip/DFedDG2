...
from vmf_utils.vmf_mixture import * 
...
class ClientVMF():
    def __init__(self, id, args, adj, dataset_train=None, user_data_idx_train=None, dataset_test=None, user_data_idx_test=None, cluster=None):
        ...
        self.train_sample_per_class = torch.zeros(self.num_classes)
        self.test_sample_per_class = torch.zeros(self.num_classes)
        
        print(f'Cluster:{self.cluster}')
        for x, y in self.train_loader:
            for yy in y:
                self.train_sample_per_class[yy.item()] += 1
        print(f'Client id: {self.id}, Train dist: {self.train_sample_per_class}')
        self.id_labels = [i for i in range(self.num_classes) if self.train_sample_per_class[i]>0]
        ...
        self.centroids = self.init_centroid()                #kmeans initialization/random/classwise average
        self.weights = self.init_weights(force_weights=None) #1/n_cluster
        self.kappas = self.init_kappas()                     #initially 1
        self.posterior_type = args.posterior_type            #"soft"
        self.vmf_soft = VonMisesFisherMixture(n_clusters=self.num_classes, weights = self.weights, centroids=self.centroids, kappas = self.kappas, parent = self, posterior_type='soft') # n_clusters=len(self.id_labels) n_init = how mamny time kmeans for initialization of centroids
    ...
    def update(self):
        ...
        print(f'Avg Local Client Update Loss LCE:{avg_CE_loss}\n')
        ...
        # self.kappa_hats = self.get_local_unc()
    ...
    def init_centroid(self):
        ...
        centroids = torch.zeros(self.num_classes,self.feat_dim).to(self.device)                                          #Unknown protos will be zeros
        
        for cc in self.id_labels:
            centroids[cc,:] = self.local_protos[cc]
            # print(centroids[cc,:])

        # print('All local centroids in array... \n')
        # for cc in range(self.num_classes):
        #     print(centroids[cc,:])
        print(f'Centroid initialized for client {self.id}.')
        return centroids

        


    def init_weights(self, force_weights):
        # init weights (alphas or pis)
        n_clusters = self.num_classes
        if force_weights is None:
            weights = torch.ones((n_clusters,)) / n_clusters
            # print(f'Component weight(pis) initialized: {weights}')
        else:
    ...
    def init_kappas(self):
        n_clusters = self.num_classes
        kappas = torch.ones((n_clusters,))
        print(f'Kappas initialized for client {self.id}.')
        return kappas
