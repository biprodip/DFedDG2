...
class ClientFedAvg():
    def __init__(self, id, args, adj, dataset_train, user_data_idx_train, dataset_test=None, user_data_idx_test=None, cluster=None):
        ...
        LOGGER.info("Train dataset size: %s", len(self.train_loader.dataset))
        LOGGER.info("Test dataset size: %s", len(self.test_loader.dataset))
    ...
    def update(self):
        ...
        for step in range(max_local_steps):
            ...
            for (x, y, indices) in self.train_loader:
                x=x[0]
        ...
        # self.model.cpu()
        if self.data_type in ['pc','img']:
            LOGGER.info("Local Train ACC: %s", np.mean(mean_correct_pc))
    ...
    def avg_model(self,run_agg_model_sd,run_agg_n_k):
        ...
        for key in run_agg_model_sd.keys():
            run_agg_model_sd[key] = run_agg_model_sd[key].float() / run_agg_n_k
    ...
    # Function to count the number of parameters that need to be sent
    def count_params(self, model=None):
    ...
    def performance_test(self, data_loader=None, saveFlag=0):
        'Performance on local test data with entropy-based uncertainty saving.'
        ...
        with torch.no_grad():
            for (x, y, indices) in data_loader:
                ...
                unc += entropy.sum().item()

                # Compute accuracy
                pred_choice = output.data.max(1)[1]
                acc += (pred_choice == y).sum().item()

                # Prepare data for saving if saveFlag is set
                if saveFlag == 1:
                    for i in range(y.shape[0]):
                        predicted_class = pred_choice[i].item()
                ...
                # Append probabilities and labels for AUC computation
                y_prob.append(probabilities.detach().cpu().numpy())
                nc = self.num_classes
                if self.num_classes == 2:
                    nc += 1
                lb = label_binarize(y.detach().cpu().numpy(), classes=np.arange(nc))
        ...
        LOGGER.info("Local (pc code style) Test performance: %s", acc / len(data_loader.dataset))

        # Save results to file if saveFlag is set
        if saveFlag == 1:
            client_id = self.id if hasattr(self, 'id') else 'unknown_client'
            save_path = f"gossip_predictions_uncertainties_client_{client_id}.json"
        ...
        avg_unc = unc / len(data_loader.dataset)

        return acc / len(data_loader.dataset), auc, avg_unc
