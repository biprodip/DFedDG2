...
class ClientDecoodAvg():
    ...
    def update(self):
        ...
        for epoch in range(max_local_epochs):
            ...
            for i, (x, y) in enumerate(trainloader):
                ...
                #loss = loss_CE + self.lamda *(lc + ld)   #self.w

                #loss =  loss_CE + .2 * lc + .1* ld   #best till now!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!********!!!!!!!!!!!!
                #loss =  loss_CE + .2 * lc + .01* ld   #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*********!!!!!!!!!!!!!!!
                
                loss =  loss_CE + lc +  ld     # 
                #loss =   lc + .2 *ld                       #CIDER #Bad
                # loss =  .8 * lc + .2 * ld
                #loss = loss_CE
                # loss = loss_CE + 0.2 * lc
    ...
    def avg_body(self,run_agg_model_sd,run_agg_n_k):
        print('Only base aggregated.')
        #Ni = np.sum(adj[self.id,:]>0)+1 #since (i,i)=0, to count self(+1)
        for key in run_agg_model_sd.keys():
            run_agg_model_sd[key] /= run_agg_n_k
        #self.agg_count = 1

        self.model.base.load_state_dict(run_agg_model_sd)
    ...
    def avg_head(self,run_agg_model_sd,run_agg_n_k):
        print('Only head aggregated.')
        #Ni = np.sum(adj[self.id,:]>0)+1 #since (i,i)=0, to count self(+1)
        for key in run_agg_model_sd.keys():
            run_agg_model_sd[key] /= run_agg_n_k
        #self.agg_count = 1

        self.model.head.load_state_dict(run_agg_model_sd)
        #print(f'Client {self.id} averaged.')
        self.req_avg = False    
