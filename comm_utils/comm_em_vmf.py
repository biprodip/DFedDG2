...
def receive_protos(client,sel_clients):
    '''Every client has multiple proto(ID)'''
...
def receive_kappas(client,sel_clients):
    '''Every client has multiple based on kappas(components)(ID)'''
    ...
    for cl in sel_clients:
        uploaded_ids.append(cl.id)
        uploaded_kappas.append(cl.local_kappas)

    
    uploaded_kappas.append(client.local_kappas) #self protos for aggregation
    uploaded_ids.append(client.id)
    return uploaded_protos,uploaded_ids
...
def proto_aggregation(local_protos_list,verbose = True):
    '''sum(proto_of_class_i)/count(proto_of_class_i)'''
...
def proto_aggregation_vmf(local_protos_list,verbose = True):
    '''sum(proto_of_class_i)/count(proto_of_class_i)'''
    agg_protos_label = defaultdict(list)
...
def comm_em_vmf(args, adj, clients, debug=False, test_loader=None):
    '''
    ...
    for e in range(5):
        for m in clients:
            m.update() #local solver


        
    for e in range(args.num_rounds):

        #(Performance debugging) check if continue learning or break
