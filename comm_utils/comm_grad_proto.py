...
def receive_protos(sel_clients):
...
def comm_grad_proto(args, adj, clients, debug=False, test_loader=None):
    '''
    ...
    for e in range(args.num_rounds):
        ...
        #receive protos
            if req_aggregation[i]:
                ...
                rec_protos = receive_protos(neighbors) #neighbors adj_clients
                #aggregate protos
                ...
                #global_protos_label =  proto_aggregation(rec_protos)  #####************
                #global_protos_label =  F.normalize(global_protos_label, dim=1)###############******************
  
                #clients[i].set_protos(global_protos_label)   #*****************
