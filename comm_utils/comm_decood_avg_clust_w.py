...
def receive_protos(client, sel_clients, adj):
    '''
    parameters:
    client: client object
    sel_clients: list of clients to receive protos from
    adj: gossip matrix
    
    computes the uploaded protos from the selected clients
    weighted uploaded protos from neighbors
    '''
    ...
    for cl in sel_clients:
        weighted_protos = {label: proto * adj[client.id][cl.id] for label, proto in cl.local_protos.items()}
        uploaded_protos.append(weighted_protos)
    
    # Self protos for aggregation
    self_weighted_protos = {label: proto * adj[client.id][client.id] for label, proto in client.local_protos.items()}
    uploaded_protos.append(self_weighted_protos)
...
def proto_aggregation(local_protos_list):
    '''
    parameters:
    local_protos_list: list of local protos
    
    computes the normalized aggregated(weighted sum) protos
    '''
    agg_protos_label = defaultdict(list)

    for local_protos in local_protos_list:   #every client protos set 
        for label, proto in local_protos.items():    #one by one weighted prototype based on labels
            agg_protos_label[label].append(proto.data)

    for label, proto_list in agg_protos_label.items():
        proto_sum = sum(proto_list)
        tmp_proto = F.normalize(proto_sum, dim=0)
...
def comm_decood_avg_clust_w(args, adj, clients, debug=False, test_loader=None):
    '''
    ...
    for e in range(args.num_rounds):
        ...
        tot_train_data = 0 #############
        new_models = []
        Ni = [1 for i in range(len(clients))] #selected neighbor length of client i every iteration(we devide this by  corresponding number during averaging)
        
        
        #############*********************************************####################

        #copy self models (full nodel)
        ...
        #head only
        # for i in range(len(clients)):
        #   new_models.append(copy.deepcopy(clients[i].model.head.state_dict()))    


        # #body only
        # for i in range(len(clients)):
        #   new_models.append(copy.deepcopy(clients[i].model.base.state_dict()))    
        ...
        #receive protos
            if req_aggregation[i]:
                ...
                id_neighbors = [adj_clients[c] for c in range(len(adj_clients)) if (adj_clients[c].id_labels == clients[i].id_labels)]
                #print(f' Neighbors: {len(id_neighbors)} ID(Same label) neighbors selected from {len(adj_clients)} peers.')
                ...
                s_c = [c.id for c in id_neighbors]
                # for peer in s_c:
                ...
                #full model
                #self weight
                # aggregate parameters using adj(gossip) matrix not using local sample size
                ...
                #Head only
                # aggregate parameters using adj(gossip) matrix not using local sample size
                # for key in clients[i].model.head.state_dict().keys():
                #     new_models[i][key] = adj[i][i] * clients[i].model.head.state_dict()[key]
                
                # for sc in neighbors:
                #     for key in sc.model.head.state_dict().keys():
                #         new_models[i][key] += adj[i][sc.id] * sc.model.head.state_dict()[key]


                #Body only
                # # aggregate parameters using adj(gossip) matrix not using local sample size
                # for key in clients[i].model.base.state_dict().keys():
                #     new_models[i][key] = adj[i][i] * clients[i].model.base.state_dict()[key]
                
                # for sc in neighbors:
                #     for key in sc.model.base.state_dict().keys():
                #         new_models[i][key] += adj[i][sc.id] * sc.model.base.state_dict()[key]
                ...
                if args.clustered_agg:
                    ...
                    rec_protos = receive_protos(clients[i], neighbors, adj) #neighbors adj_clients
        ...
        for i in range(len(clients)):
                ...
                #head only  
                #clients[i].avg_head(new_models[i], 1) #New models already contains weighted sum

                #only body
                #clients[i].avg_body(new_models[i], 1) #New models already contains weighted sum
