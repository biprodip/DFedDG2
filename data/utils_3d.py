...
def load_modelnet_data(datadir, num_category=10):

    modelnet_train_ds = ModelNetDataset(datadir, train=True, num_category=num_category)
    modelnet_test_ds = ModelNetDataset(datadir, train=False, num_category=num_category)

    X_train, y_train = modelnet_train_ds.list_of_points, modelnet_train_ds.list_of_labels
    X_test, y_test = modelnet_test_ds.list_of_points, modelnet_test_ds.list_of_labels
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if dataset in ['modelnet10', 'modelnet40']:
        if '10' in dataset: 
            X_train, y_train, X_test, y_test = load_modelnet_data(datadir, num_category=10)
        else:
            X_train, y_train, X_test, y_test = load_modelnet_data(datadir, num_category=40)
    elif dataset == 'fmnist':
    ...
    with torch.no_grad():
        for tmp in dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                x, target = process_3d_pc(x, target)
                ...
                if x.shape[-2] in [3, 6]:  # 3D Point Cloud Data
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0):
    if dataset in ('mnist', 'modelnet10', 'modelnet40', 'femnist', 'fmnist', 'cifar10', 'svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY', 'cifar100', 'tinyimagenet'):
        if dataset == 'mnist':
...
def process_3d_pc(points, target):
