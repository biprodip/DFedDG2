...
from lib.data_utils import TwoCropTransform
...
def prepare_data_office_noniid(num_users, args):
    ...
    if args.model_type == 'cnn' or args.model_type == 'other':
        transform_office = transforms.Compose([
            transforms.Resize([64, 64]),
            transforms.ToTensor(),
        ])
...
def prepare_data_real_noniid(num_users, args):
    ...
    for k in range(K):
        idx_k = np.where(y == k)[0]
        LOGGER.debug("len(idx_k): %s", len(idx_k))
...
def visualize_data_dist_fnli(args, df, K, num_users):
    ...
    for artist in g.legend.legend_handles:  #g.legend.legendHandles
        artist.set_edgecolor(".7")
    g.savefig('./noniid_train_'+ str(args.num_classes) + 'c_' +str(num_users)+ 'u_' +'a' +str(args.alpha)+'_f'+str(args.feature_iid)+'_l'+str(args.label_iid)+'.pdf', format='pdf')
...
def recon(args, backbone_list, model):
    ...
    for iter in range(300):
        ...
        LOGGER.debug("Iteration : %s\tLoss: %.3f", iter, dummy_loss.item())
        dummy_loss.backward()
...
def exp_details(args):
    ...
    LOGGER.info("    Learning  : %s", args.lr)
    LOGGER.info("    Global Rounds   : %s\n", args.rounds)

    LOGGER.info("    Federated parameters:")
    if args.label_iid:
        LOGGER.info("   Label IID")
    else:
        LOGGER.info("   Label Non-IID")
    if args.feature_iid:
        LOGGER.info("   Feature IID")
    else:
        LOGGER.info("   Feature Non-IID")
    LOGGER.info("    Local Batch size   : %s", args.local_bs)
    LOGGER.info("    Local Epochs       : %s\n", args.train_ep)
    return
