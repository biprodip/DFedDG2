...
def main():
     ...
     #save adjacency matrics
          ...
          for c in clients:
               #save client_models
               filename = args.params_dir + 'checkpoint_{}_{}_{}_client_{}.pth.tar'.format(args.algorithm, args.dataset, args.comm, c.id)
               torch.save({
               ...
               with open(filename, 'wb') as f:
                    if args.algorithm in ['FedAvg','Ditto']:
                         pickle.dump([c.train_loader, c.test_loader, c.id_labels, c.l_test_acc_hist], f)
                    else:
                         pickle.dump([c.train_loader, c.test_loader, c.id_labels, c.local_protos, c.global_protos, c.l_test_acc_hist], f)
